...
const TimelineWindow = ({ tripDates, activityDict, handler }) => {
  const [itinerary, setItinerary] = useState(activityDict);
    ...
    <ol className="relative border-s border-gray-200 dark:border-gray-700">
      {tripDates.map((date, index) => (
        <li key={index} className="mb-10 ms-4">
          ...
          {
            itinerary[date] > 0 ?
              <p> There are activities </p> :
              //create a button that when clicked, opens a modal that allows the user to add an activity to the list
              <div>
              <button onClick={() => handler(date)} className="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:outline-none focus:ring-gray-200 focus:text-blue-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 dark:focus:ring-gray-700">Add Activity
